...
try:
    ...
    logger.warning(f"HSReplay archetype data unavailable, fallback to class names: {e}")
    not_classify = True
...
async def _update_archetypes_loop():
    """Background task: refresh deck archetypes daily at 6 AM."""
    while True:
        ...
        if now.hour == 6 and now.minute == 0:
            try:
                ...
                logger.error(f"Archetype update failed: {traceback.format_exc()}")
            await asyncio.sleep(3600)  # wait an hour after update


async def _do_update():
    global clusters, cluster_map, deck_data
    async with httpx.AsyncClient() as client:
        cluster_data = (await client.get(cluster_url)).json()
        header = {"accept-language": "zh-CN,zh"}
        deck_data = (await client.get(deck_url, headers=header)).json()
    clusters, cluster_map = init_clusters(cluster_data)
    logger.info("Archetype data updated successfully")
